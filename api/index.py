...
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBma25tdmZyc2l6c2R2eGtuam1tIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI4MjM1NjgsImV4cCI6MjA4ODM5OTU2OH0.HFwO-IcBFdqkU6CITuDKg8jMCLbGsQN0VrU4pgiXJfs"
...
:root{
  --navy:#1b3a5c;--navy-dark:#152e4a;--red:#c0392b;
...
.logo-maintenance{font-size:1.05rem;font-weight:800;color:var(--white)}
...
.form-control{width:100%;padding:9px 12px;border:1.5px solid var(--gray-300);
  ...
  transition:border-color var(--t);appearance:none;-webkit-appearance:none}
...
def dashboard():
    ...
    return render(T_DASHBOARD, total=total, working=working, in_repair=in_repair,
                  in_storage=in_storage, recent=obj(recent_raw), accounts=get_accounts())
...
def edit_equipment(item_id):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("equipment"))
    return render(T_EQUIPMENT_FORM, action="Edit", item=obj(item_raw), accounts=accts)
...
def add_maintenance():
    ...
    if request.method == "POST":
        ...
        if existing:
            update = {}
            cur = parse_date(existing.get("last_service_date"))
            new = parse_date(sd)
            if cur is None or (new and new > cur):
